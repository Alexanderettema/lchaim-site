<?php
    ...
    while ( have_posts() ) : the_post();
        ...
        $pofo_hide_icon_archive = get_theme_mod( 'pofo_hide_icon_archive', '1' );
        ...
        echo '<div class="blog-post-style-archive blog-post-style3 '.$class_column.$pofo_post_classes_infinite_scroll.'"'.$pofo_animation_delay_attr.'>';
            echo '<div '.$pofo_post_classes.'>';
                echo '<div class="grid-item">';
                    echo '<div class="blog-post bg-light-gray inner-match-height">';
                        ...
                        echo '<div class="post-details padding-ten-all sm-padding-20px-all xs-text-center pofo-post-description pofo-box-background-color">';
                            echo '<div class="blog-hover-color"></div>';
                            if( $pofo_show_post_title_archive == 1 ){
                                $page_url = get_permalink();
                                echo '<a class="alt-font post-title text-medium text-extra-dark-gray display-block entry-title'.$pofo_title_dynamic_font_size.$pofo_blog_post_title_text_transform_archive.'" href="'.esc_url( $page_url ).'"'.$pofo_title_style_attr.'>'.get_the_title().'</a>';
                            }
                            ...
                            if( !empty( $author_date_array ) ){
                                echo '<div class="author text-extra-small text-medium-gray display-block margin-20px-top sm-margin-15px-top">';
                                    echo '<span class="text-medium-gray text-extra-small display-inline-block'.$pofo_blog_post_meta_text_transform_archive.'">'.implode( '<span class="blog-separator vertical-align-middle">|</span>', $author_date_array ).'</span>';
                                echo '</div>';
                            }
