<?php
    ...
    while ( have_posts() ) : the_post();
        ...
        ob_start();
            post_class('blog-post-style-archive grid-item '.$class_column.$pofo_post_classes_infinite_scroll.$pofo_zoom_effect_archive);
            ...
            echo '<figure>';
                ...
                echo '<figcaption>';
                    echo '<div class="portfolio-hover-main text-left">';
                        echo '<div class="blog-hover-box vertical-align-bottom pofo-post-description">';
                            if( !empty( $author_date_array ) ){
                                echo '<span class="post-author text-extra-small text-medium-gray display-block margin-5px-bottom xs-margin-5px-bottom'.$pofo_blog_post_meta_text_transform_archive.'">'.implode( '<span class="blog-separator display-inline-block vertical-align-middle">|</span>', $author_date_array ).'</span>';
                            }
                            if( $pofo_show_post_title_archive == 1 ){
                                echo '<h6 class="alt-font display-block text-white font-weight-600 no-margin-bottom">';
                                    echo '<a href="'.get_the_permalink().'" class="text-white entry-title'.$pofo_title_dynamic_font_size.$pofo_blog_post_title_text_transform_archive.'"'.$pofo_title_style_attr.'>'.get_the_title().'</a>';
                                echo '</h6>';
                            }
                            ...
                            if( $pofo_show_button_archive == 1 ){
                                $page_url = get_permalink();
                                echo '<a href="'.esc_url( $page_url ).'" class="btn-white btn btn-very-small no-margin-bottom margin-10px-top white-space-normal">'.$pofo_button_text_archive.'</a>';
                            }
                        echo '</div>';
                    echo '</div>';
                echo '</figcaption>';
