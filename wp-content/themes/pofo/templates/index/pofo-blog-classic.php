<?php
    ...
    switch ( $pofo_blog_type_default ) {
        ...
        default:
            $pofo_general_default_type_settings .= ' margin-50px-bottom xs-margin-30px-bottom';
            break;
    }
    while ( have_posts() ) : the_post();
        if( !is_sticky() ) {
            ...
            /* Check text transform for meta to show */
            ...
            echo '<div class="blog-post-style-default blog-post blog-post-style1 xs-text-center'.$class_column.$pofo_post_classes_infinite_scroll.$pofo_zoom_effect_default.'"'.$pofo_animation_delay_attr.'>';
                echo '<div '.$pofo_post_classes.'>';
                    echo '<div class="pofo-default-post-description">';
                        ...
                        echo '<div class="post-details display-inline-block">';
                            if( !empty( $author_date_array ) ){
                                echo '<span class="post-author text-extra-small text-medium-gray display-block margin-10px-bottom xs-margin-5px-bottom'.$pofo_blog_post_meta_text_transform_default.'">'.implode( '<span class="blog-separator vertical-align-middle">|</span>', $author_date_array ).'</span>';
                            }
                            if( $pofo_show_post_title_default == 1 ){
                                $page_url = get_permalink();
                                echo '<a class="alt-font post-title text-medium text-extra-dark-gray display-block width-90 sm-width-100 entry-title'.$pofo_title_dynamic_font_size.$pofo_blog_post_title_text_transform_default.'" href="'.esc_url( $page_url ).'"'.$pofo_title_style_attr.'>'.get_the_title().'</a>';
                            }
                        echo '</div>';
                        ...
                        if( $pofo_show_like_default == 1 || $pofo_show_comment_default == 1 ){
                            echo '<div class="blog-like-comment pofo-blog-post-meta margin-20px-top sm-margin-15px-top'.$pofo_blog_post_meta_text_transform_default.'">';
