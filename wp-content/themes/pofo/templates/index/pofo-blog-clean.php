<?php
    ...
    switch ( $pofo_blog_type_default ) {
        ...
        default:
            $pofo_general_default_type_settings .= ' margin-80px-bottom sm-margin-50px-bottom xs-margin-30px-bottom';
            break;
    }
    while ( have_posts() ) : the_post();
        if( !is_sticky() ) {
            ...
            if( $pofo_show_post_date_default == 1 ) {
                $author_date_array[] = '<span class="pofo-blog-post-meta text-extra-small text-medium-gray display-inline-block published">'.get_the_date( $pofo_date_format_default, get_the_ID()).'</span><time class="updated display-none" datetime="'.get_the_modified_date( 'c' ).'">'.get_the_modified_date( $pofo_date_format_default ).'</time>';
            }
            echo '<div class="'.$class_column.$pofo_post_classes_infinite_scroll.'"'.$pofo_animation_delay_attr.'>';
                echo '<div class="blog-post blog-post-style2 xs-text-center">';
                    if ( !post_password_required() ) {
                        if( $pofo_show_post_thumbnail_default == 1 || $pofo_show_category_default == 1 ){
                    ...
                    echo '<div class="post-details pofo-default-post-description">';
                        if( $pofo_show_post_title_default == 1 ){
                            echo '<a href="'.get_the_permalink().'" class="post-title text-medium text-extra-dark-gray display-block margin-15px-bottom entry-title'.$pofo_title_dynamic_font_size.$pofo_blog_post_title_text_transform_default.'"'.$pofo_title_style_attr.'>'.get_the_title().'</a>';
                        }
                        ...
                        if( !empty( $author_date_array ) ){
                            echo '<div class="author margin-20px-bottom xs-margin-15px-bottom text-extra-small text-medium-gray'.$pofo_blog_post_meta_text_transform_default.'">';
                                echo implode( '<span class="blog-separator">|</span>', $author_date_array );
                            echo '</div>';
                        }
                        if( $pofo_show_like_default == 1 || $pofo_show_comment_default == 1 ){
                            echo '<div class="pofo-blog-post-meta blog-like-comment margin-20px-bottom sm-margin-15px-bottom'.$pofo_blog_post_meta_text_transform_default.'">';
                        ...
                        if( $pofo_show_button_default == 1 ){
                            $page_url = get_permalink();
                            echo '<a href="'.esc_url( $page_url ).'" class="btn-black btn btn-very-small no-margin white-space-normal">'.$pofo_button_text_default.'</a>';
                        }
                    echo '</div>';
                echo '</div>';
            echo '</div>';
